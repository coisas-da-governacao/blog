genericSummary2 <- function(text,k,split=c(".","!","?"),min=5,breakdown=FALSE,...){
  ...
  sentences <- sentences[stringr::str_count(sentences, "\\S+") > min]
  ...
  Vt <- RSpectra::svds(A, k = 5)$v
  rownames(Vt) <- colnames
